...
void loop() {
  ...
  if (trunk_key() == true){                // trunck key 눌림
    ...
    if (trunk_key_judge() == true)   event_key_input();          // 100ms간 눌림 확인. 조건 미충족시 바로 return FALSE 로 나올 것
    delay(100);
  }
  ...
  if (flag_state == STATE_RUN){                       // STATE_RUN      구동 중인 상태
    ...
    if (flag_open == OPEN){                           // 열리는 상태
      ...
      stepper_cnt++;
      if(stepper_cnt >= ANGLE_MAX) {flag_state = STATE_IDLE; flag_open = OPEN; stepper_cnt = ANGLE_MAX;}     // 스텝모터가 목표위치 도달하면 구동 완료 상태로 변경
    }
    else{                                             // 닫히는 상태
      set_stepper(CLOSE);                             // 열리는 포지션으로 가는 코드
      stepper_cnt--;
      if(stepper_cnt <= ANGLE_MIN) {flag_state = STATE_IDLE; flag_open = CLOSE; stepper_cnt = ANGLE_MIN;}    // 스텝모터가 목표위치 도달하면 구동 완료 상태로 변경
    }
